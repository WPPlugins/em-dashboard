...
class EM_Dashboard_Styles extends EM_Dashboard_Init {
	...
	public function em_button_styles() {
		...
		if ( $load_easy && !$forced ) {
			if ( is_rtl() ) {
				wp_register_style( 'em-button-rtl', EM_ADMIN_BUTTON_RTL, array( 'dashicons' ), EM_PLUGIN_VER, 'all' );
				wp_enqueue_style( 'em-button-rtl' );
			} else {
	 ...
	 * @uses EM_PLUGIN_VER
	 *
	 * @global $wp_customize Checks if customizer is active
	 *
	...
	public function dashboard_styles() {
		...
		//* Load default plugable styles
		// Always load on HMPL
		if ( !isset( $wp_customize ) && ( $em_styles || ( defined( 'USE_EM_DASHBOARD_THEME') && USE_EM_DASHBOARD_THEME ) || ( defined( 'IS_HMPL' ) && IS_HMPL ) ) ) {
			/**
			 * Removes Default WP Admin styles
			 *
			 * @since 1.0.3
			 */
			...
			if ( is_rtl() ) {
				wp_register_style( 'em-dashboard-rtl', EM_ADMIN_STYLE_RTL, array( 'wp-admin', 'buttons', 'open-sans', 'dashicons' ), EM_PLUGIN_VER, 'all' );
				wp_enqueue_style( 'em-dashboard-rtl' );
			} else {
				wp_register_style( 'em-dashboard', EM_ADMIN_STYLE, array( 'wp-admin', 'buttons', 'open-sans', 'dashicons' ), EM_PLUGIN_VER, 'all' );
		...
		if ( ( $em_styles && $em_fonts ) || ( defined( 'USE_EM_DASHBOARD_THEME') && USE_EM_DASHBOARD_THEME ) || ( defined( 'IS_HMPL' ) && IS_HMPL ) ) {
			 ...
			 */
			if ( 'off' !== _x( 'on', 'Open Sans & Merriweather font: on or off', 'em-dashboard' ) ) {
				$subsets = 'latin,latin-ext';

				/* translators: To add an additional Open Sans & Merriweather character subset specific to your language,
				 * translate this to 'greek', 'cyrillic' or 'vietnamese'. Do not translate into your own language.
				 */
				$subset = _x( 'no-subset', 'Open Sans font: add new subset (greek, cyrillic, vietnamese)', 'em-dashboard' );
