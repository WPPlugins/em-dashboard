...
class EM_Dashboard_OptionsPage extends EM_Dashboard_Options {
	...
	public function register_fields() {
		...
		register_setting( 'em_dashboard_g_settings', 'em_dashboard_force_easy_capability', 	array( $this, 's_capability' ) ); //default install_plugins
		...
		add_settings_field (
			...
			__( 'EM Dashboard Styles', 'em-dashboard' ),		// title
			...
			array( 												// args
				...
				'default' 	=> 'em_styles',
				'labelinfo'	=> __( 'Load the EM Dashboard theme.', 'em-dashboard' ),
				'hoverinfo'	=> __( 'Experimental flat soft WP theme (this also removes the color picker)', 'em-dashboard' ),
				'hoverinfosite'	=> '',
				'hoverinfonetwork' => '',
				'class'		=> 'alternate',
			)
		...
		add_settings_field (
			...
			__( 'EM Dashboard Fonts', 'em-dashboard' ), 	// title
			array( $this, 'checkbox' ),	 					// callback
			'em_dashboard_g_settings',		 	 			// page
			'em_dashboard_general_settings',				// section
			array( 											// args
				...
				'page'		=> 'em_dashboard_g_settings',
				'default' 	=> 'em_fonts',
				'labelinfo'	=> __( 'Load the Merriweather Google Font that goes with the EM Dashboard theme.', 'em-dashboard' ),
				'hoverinfo'	=> __( 'This option only has effect if the EM Dashboard Styles are also active', 'em-dashboard' ),
				'hoverinfosite'	=> '',
				'hoverinfonetwork' => '',
				'class'		=> 'alternate',
			)
		...
		add_settings_field (
			...
			__( 'Default Color Scheme', 'em-dashboard' ), 	// title
			...
			array( 											// args
				...
				'labelinfo'	=> __( 'Set the default theme for newly registered users.', 'em-dashboard' ),
				'hoverinfo'	=> __( 'This does not affect currently registered users.', 'em-dashboard' ),
				'hoverinfosite'	=> '',
				'hoverinfonetwork' => '',
				'class'		=> '',
			)
		...
		add_settings_field (
			...
			__( 'Enable Color Picker', 'em-dashboard' ), // title
			array( $this, 'checkbox' ),					// callback
			'em_dashboard_g_settings',			 	 	// page
			'em_dashboard_general_settings',			// section
			array( 										// args
				...
				'labelinfo'	=> __( 'When this option is on, the color picker is visible in the user profile page.', 'em-dashboard' ),
				'hoverinfo'	=> '',
				'hoverinfosite'	=> __( "This option is set to on if 'Enable EM Dashboard Styles' is on. This has no effect on the admin", 'em-dashboard' ),
				'hoverinfonetwork'	=> __( "This option is set to on if 'Enable EM Dashboard Styles' is on. This has no effect on the super-admin", 'em-dashboard' ),
				'class'		=> '',
			)
		);
		add_settings_field (
			'em_dashboard_load_easy', 							// id
			__( 'Enable Easy Mode Button', 'em-dashboard' ),   	// title
			array( $this, 'checkbox'), 							// callback
			'em_dashboard_g_settings',		 	 				// page
			'em_dashboard_general_settings',					// section
			array( 												// args
				...
				'page'		=> 'em_dashboard_g_settings',
				'default' 	=> 'load_easy',
				'labelinfo'	=> __( 'Enable the easy mode button.', 'em-dashboard' ),
				'hoverinfo'	=> '',
				'hoverinfosite'	=> __( 'The button on the bottom left.', 'em-dashboard' ),
				'hoverinfonetwork'	=> __( "The button on the bottom left. This doesn't have effect in the network admin screen", 'em-dashboard' ),
				'class'		=> 'alternate',
			)
		);
		add_settings_field (
			'em_dashboard_easy_mode', 						// id
			__( 'Default Easy Mode Setting', 'em-dashboard' ), // title
			array( $this, 'dropdown'), 						// callback
			'em_dashboard_g_settings',		 	 			// page
			'em_dashboard_general_settings',				// section
			array( 											// args
				...
				'page'		=> 'em_dashboard_g_settings',
				'default' 	=> 'easy_mode',
				'labelinfo'	=> __( 'Turn the button on the bottom left on or off for new users.', 'em-dashboard' ),
				'hoverinfo'	=> __( "This option only has effect on users who haven't touched it yet", 'em-dashboard' ),
				'hoverinfosite'	=> '',
				'hoverinfonetwork' => '',
				'class'		=> 'alternate',
			)
		...
		add_settings_field (
			...
			__( 'Force Easy Mode', 'em-dashboard' ), 		// title
			array( $this, 'checkbox'), 					// callback
			'em_dashboard_g_settings',		 			// page
			'em_dashboard_general_settings',			// section
			array( 										// args
				...
				'labelinfo'	=> __( "Force Easy Mode for everyone except for the set Capability below.", 'em-dashboard' ),
				'hoverinfo'	=> __( 'This option also removes the button on the bottom left except for the minimum capability.', 'em-dashboard' ),
				'hoverinfosite'	=> '',
				'hoverinfonetwork' => '',
				'class'		=> '',
			)
		...
		add_settings_field (
			...
			__( 'Minimum Non-Forced Capability', 'em-dashboard' ),	// title
			array( $this, 'dropdown'),								// callback
			'em_dashboard_g_settings',				 				// page
			'em_dashboard_general_settings',			 			// section
			array( 													// args
				...
				'labelinfo'	=> __( "The minimum role that doesn't get Easy Mode forced and sees the Easy Mode Button at all times regardless of other settings.", 'em-dashboard' ),
				'hoverinfo'	=> __( "Although very unlikely, if you get stuck in Easy Mode, please see the plugin page's 'Other Nodes'.", 'em-dashboard' ),
				'hoverinfosite'	=> '',
				'hoverinfonetwork' => '',
				'class'		=> '',
			)
	...
	public function add_menu_link() {
		...
		$pagetitle = __( 'EM Dashboard Settings', 'em-dashboard' );
	...
	public function menu_page() {
		...
		$title = __( 'EM Dashboard Settings', 'em-dashboard' );
		...
		<div class="wrap">
			...
			if ( isset( $_GET['updated'] ) ) { ?>
				<div id="message" class="updated notice is-dismissible">
					<p>
						<?php _e( 'Your settings have been saved', 'em-dashboard' ) ?>
					</p>
				</div>
			<?php
			}
			...
			$url = add_query_arg( 'page', 'em_dashboard_settings', $url );
			$url = add_query_arg( 'tab', 'general_options', $url );
			$url = esc_url_raw( $url );

			?>
			<h2 class="nav-tab-wrapper">
				<a href="<?php echo $url; ?>" class="nav-tab <?php echo $active_tab == 'general_options' ? 'nav-tab-active' : ''; ?>">
					<?php _e( 'General Settings', 'em-dashboard' ); ?>
				</a>
				<!--<a href="?page=em_dashboard_settings&tab=easy_options" class="nav-tab <?php echo $active_tab == 'easy_options' ? 'nav-tab-active' : ''; ?>">
					<?php _e( 'Easy Mode Settings', 'em-dashboard' ); ?>
				</a>
				<a href="#" class="nav-tab " style="pointer-events:none;">
					<?php _e( 'More coming soon!', 'em-dashboard' ); ?>
				</a>-->
			</h2>
	...
	/**
	 * Register general settings and add welcome field.
	 *
	 * @since 1.0.0
	 */
	public function general_settings_fields() {
		...
		if ( is_multisite() ) {
			$welcome = __( 'The settings on this page affect every user and site on your network.', 'em-dashboard' );
		} else {
			$welcome  = __( 'The settings on this page affect every user on your website', 'em-dashboard' );
		}

		?><p style="display:inline-block;width:auto;padding:10px;margin:3px 0;border:1px solid #aaa;"><?php echo $welcome; ?></p><?php
	...
	public function dropdown( $args = array() ) {
		...
		// Start Force Easy Capabilities
			...
			if ( is_multisite() ) {
				$caps = array(
					'install_plugins' 			=> __( 'Super Administrator', 'em-dashboard' ),
					'manage_options' 			=> __( 'Administrator', 'em-dashboard' ),
					'publish_pages' 			=> __( 'Editor', 'em-dashboard' ),
					'publish_posts' 			=> __( 'Author', 'em-dashboard' ),
					'edit_posts' 				=> __( 'Contributor', 'em-dashboard' ),
					'read' 						=> __( 'Subscriber', 'em-dashboard' ),
				);
			} else {
				$caps = array(
					'manage_options' 			=> __( 'Administrator', 'em-dashboard' ),
			...
			?><em>Options failed to load. Please report to the plugin author.</em><?php
			return;
		}
	...
	public function s_capability( $value, $default = '' ) {
		...
		$caps = array( 'install_plugins', 'manage_options', 'publish_pages', 'publish_posts', 'edit_posts', 'read' );
