...
class EM_Dashboard_Init {
	...
	public function __construct() {
		...
		add_action( 'admin_init', array( $this, 'easy_mode_switch' ) );
	...
	public function easy_mode_on( $easy = array() ) {
		...
		if ( current_user_can( $capability ) && ( $rescue || ! $load_easy ) ) {
			return $mode_cache = false;
		} else {
			return $mode_cache = true;
		}
	...
	public function rescue() {
		 ...
		 * add_filter( 'em_dashboard_forced_rescue', '__return_true');
		 */
		$rescue = (bool) apply_filters( 'em_dashboard_forced_rescue', false );

		if ( $rescue )
			return $rescue_cache = true;

		// Yes, I've optimized a rescue function. Now hire me.
		$option = $this->option();
	...
	public function easy_mode_switch() {
		...
		if ( $user_id != 0 ) {
			...
			/**
			 * get_query_var isn't allowed at this point. Requires init.
			 *
			...
			if ( $query ) {
				if ( $query === 'on' ) {
					$update = update_user_meta( $user_id, $option_name, $query, 'off' );
				} else if ( $query === 'off' ) {
					$update = update_user_meta( $user_id, $option_name, $query, 'on' );
				}
				...
				if ( $update ) {
					...
					$url = remove_query_arg( 'easy-mode', $url );
	 ...
	 */
	public function admin_redirect( $page, $query_args = array() ) {
	 ...
	 *
	 * @param bool $forced
	 * @param string $capability minimum capability ( default: install_plugins );
	 *
	 * @param bool $rescue used if the user is stuck to gain access back again to the dashboard
	 *
	 * @staticvar bool $forced_cache;
	 *
	 * @return bool forced
	 */
	public function forced() {
		...
		if ( isset( $forced_cache ) )
			return $forced_cache;

		$rescue = $this->rescue();

		if ( $rescue )
			return $forced_cache = false;
	...
	public function easy_mode_button() {
		...
		*/
		 ...
		 */
		if ( $load_easy && ! $forced && ! is_network_admin() ) {

			$easy_activate = esc_html__( 'Easy mode', 'em-dashboard' );
	...
	*/
			...
			$url = esc_url_raw( $url, array( $protocol ) );
			...
			$output = '<li id="easy-mode-reload" class="wp-not-current-submenu menu-top menu-top-last toplevel_page_easymode">'
					. '<div>' // has been added to prevent jQuery bugs (related to "current" page because the URL has the same url as current page.).
					. '<a href="' . $url . '" target="_self" id="easy-switch-no-js"><div id="easy-activate"><div></div></div>' // yup, block inside inline. oops? WP does it too. I blame Matt.
					. $contentPlain
					. '</a>'
					. '</div>' // has been added to prevent jQeury bugs
					. '</li>'
					;
